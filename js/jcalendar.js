...
function nextImage() {
	...
	var i = 0,
		sum = 0,
		size = selectedPosition % events.length + 1;
	while(sum < size) {
...
function preImage() {
	...
	var i = 0,
		sum = 0,
		size = selectedPosition == 1 ? events.length : selectedPosition - 1;
	while(sum < size) {
		sum += shopEvents.monthEvents[i++].events.length;
	}
	JCalendar.selected_year = shopEvents.monthEvents[i - 1].year;
...
$(document).ready(function() {
	$.get("http://localhost:54114/api/values", function(data) {
		window.shopEvents = JSON.parse(data);
		window.current_date = shopEvents.date.split("-");
		window.current_year = current_date[0],
		window.current_month = getFull(current_date[1]),
		window.current_day = getFull(current_date[2]);
		calendar_init();
		fit_height();
	});
});